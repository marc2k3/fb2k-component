...
namespace resizer
{
	...
	static HRESULT encode(Format format, IWICBitmapSource* source, album_art_data_ptr& data)
	{
		...
		if (format == Format::WEBP)
		{
			...
			RETURN_IF_FAILED(lock->GetDataPointer(&size, &in));
			RETURN_IF_FAILED(lock->GetStride(&stride));

			const size_t new_size = WebPEncodeBGRA(in, width, height, stride, 95, &out);
			if (new_size > 0)
			{
				data = album_art_data_impl::g_create(out, new_size);
				WebPFree(out);
				return S_OK;
			}
#endif
